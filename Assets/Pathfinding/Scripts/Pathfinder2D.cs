...
public class Pathfinder2D : MonoBehaviour 
{
    ...
    void Start()
    {
		...
		MapStartPosition = plane.GetComponent<Renderer> ().bounds.min;
		MapEndPosition = plane.GetComponent<Renderer> ().bounds.max;
        if (Tilesize <= 0)
        {
            Tilesize = 1;
        }
    ...
    private void Create2DMap()
    {
		...
		print(plane.GetComponent<Renderer>().bounds.center);
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
				...
				RaycastHit[] hit = Physics.SphereCastAll(new Vector3(x, y, zStart), Tilesize*2, Vector3.forward, dist, layerMask);
                bool free = true;
             ...
             //  {
			     ...
			     if (hit.Length > 0){
					...
					print("hit: " + hit[0].transform.name);
				//print("lol");



				//if (DisallowedTags.Contains(hit[0].transform.tag))
                 //  {
						//if (h.transform.position.z < zStart)
                       // {
					//print(hit[0].transform.tag);
                            //It is a disallowed walking tile, make it false
						Map[j, i] = new Node(j, i, y, ID, x, plane.transform.position.z, false); //Non walkable tile!
                            free = false;
                            //maxZ = h.point.z;
                        //}
                   // }
				//else
						if (IgnoreTags.Contains(hit[0].transform.tag))
                    {
				...
				print("MLAPAPAPA");
						...
						Map[j, i] = new Node(j, i, y, ID, x, plane.transform.position.z, true); //walkable tile!
                            free = false;
                            //maxZ = h.point.z;
                        //}
                    }
	...
	void addObstacles()
	{
		...
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				...
				RaycastHit[] hit = Physics.Linecast(transform.position, new Vector3(x, y, 10));
                 ...
                 /*   for (int y = i - 1; y < i + 2; y++)
                    {
                        for (int x = j - 1; x < j + 2; x++)
                        {
							...
							else UnityEngine.Debug.DrawLine(start, end, Color.yellow);
...
*/
					...
					Vector3 start2 = new Vector3(Map[j, i].xCoord-0.2f, Map[j, i].yCoord, Map[j, i].zCoord- 0.1f);
					Vector3 end2 = new Vector3(Map[j, i].xCoord+0.2f, Map[j, i].yCoord, Map[j, i].zCoord- 0.1f);
					...
					if (!Map[j, i].walkable)
						UnityEngine.Debug.DrawLine(start2, end2, Color.red);
					else UnityEngine.Debug.DrawLine(start2, end2, Color.green);
					
					
					
					start2 = new Vector3(Map[j, i].xCoord, Map[j, i].yCoord-0.2f, Map[j, i].zCoord- 0.1f);
					end2 = new Vector3(Map[j, i].xCoord, Map[j, i].yCoord+0.2f, Map[j, i].zCoord- 0.1f);
